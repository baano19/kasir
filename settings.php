...
// --- LOGIC CABANG (BARU) ---
if(isset($_POST["add_b"])){ $db->prepare("INSERT INTO branches (name) VALUES (?)")->execute([$_POST["n"]]); header("Location: settings.php?tab=branches"); exit(); }
if(isset($_POST["up_b"])){ $db->prepare("UPDATE branches SET name=? WHERE id=?")->execute([$_POST["n"], $_POST["id"]]); header("Location: settings.php?tab=branches"); exit(); }
...
if(isset($_POST["update_meal"])){ $db->prepare("REPLACE INTO settings (key, value) VALUES ('meal_allowance', ?)")->execute([$_POST["meal_val"]]); header("Location: settings.php?tab=profile"); exit(); }
...
<div class="sidebar" id="sidebar">
    <h2>BPOS</h2>
    <a href="dashboard.php">Dashboard</a><a href="transactions.php">Transaksi</a><a href="expenses.php">Pengeluaran</a>
...
<div class="content">
    ...
    <?php if($tab == "branches"): ?>
        <div class="card"><h3>Tambah Cabang</h3><form method="POST" style="display:flex; gap:10px;"><input type="text" name="n" placeholder="Nama Cabang (ex: Cabang Utama)" style="flex:1;" required><button name="add_b">Tambah</button></form></div>
        <div class="table-container"><table><thead><tr><th>ID</th><th>Nama Cabang</th><th>Aksi</th></tr></thead><tbody>
            <?php foreach($branches as $b): ?>
            <tr><form method="POST"><input type="hidden" name="id" value="<?=$b['id']?>"><td><?=$b['id']?></td><td><input type="text" name="n" value="<?=$b['name']?>"></td><td><button name="up_b" style="padding:5px 10px!important;">Save</button> <button name="del_b" class="del" onclick="return confirm('Hapus cabang ini? Pastikan data transaksi sudah aman!')" style="padding:5px 10px!important;">Del</button></td></form></tr>
            <?php endforeach; ?>
        </tbody></table></div>
    ...
    <?php elseif($tab == "services"): ?>
        ...
        <div class="table-container"><table><thead><tr><th>Cabang</th><th>Nama Layanan</th><th>Harga</th><th>Aksi</th></tr></thead><tbody>
            <?php $sv=$db->query("SELECT s.*, b.name as bname FROM services s LEFT JOIN branches b ON s.branch_id=b.id ORDER BY s.branch_id ASC, s.id DESC")->fetchAll(); foreach($sv as $s): ?>
    ...
    <?php else: ?>
        ...
        <div class="card" style="max-width:400px; border-left-color:orange;"><h3 style="color:orange; margin-top:0;">Pengaturan Uang Makan</h3><form method="POST" style="display:flex; gap:10px; align-items:end;"><div style="flex:1;"><label style="font-size:0.8rem; color:#ccc;">Nominal (Rp)</label><input type="number" name="meal_val" value="<?=$meal_allowance?>" required style="width:100%!important; margin:0!important; height:38px!important;"></div><button name="update_meal" style="background:orange!important; color:white!important; height:38px!important; border:none; border-radius:6px;">Simpan</button></form></div>
    <?php endif; ?>
</div>
